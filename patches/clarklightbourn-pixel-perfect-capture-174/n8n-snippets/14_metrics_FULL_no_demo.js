...
enrichSubmarketGeo(extracted, omText);
if (extracted.submarket_median_income) {
...
// Prefer LLM-extracted comps / supply when present (no demo packs).
if ((!market.comps || !market.comps.length) && Array.isArray(extracted.rent_comps)) {
  market.comps = extracted.rent_comps.map((c) => ({
    property: c.property || c.name || '',
    units: c.units ?? 0,
    year_built: c.year_built ?? 0,
    avg_rent: c.avg_rent ?? c.rent ?? 0,
    occupancy: c.occupancy ?? 0,
    distance: c.distance_miles ?? c.distance ?? 0,
  })).filter((c) => c.property);
}
if ((!market.supply || !market.supply.length) && Array.isArray(extracted.supply_deliveries)) {
  market.supply = extracted.supply_deliveries.map((d) => ({
    year: String(d.year ?? 'Pipeline'),
    deliveries: d.deliveries ?? 0,
    stock_pct: d.pct_of_stock ?? d.stock_pct ?? 0,
  }));
}
if (market.pipeline_pct_of_stock == null && extracted.pipeline_pct_of_stock != null) {
  market.pipeline_pct_of_stock = Number(extracted.pipeline_pct_of_stock);
}
if (market.pipeline_pct_of_stock != null && (!market.supply || !market.supply.length)) {
  market.supply = [{
    year: 'Pipeline',
    deliveries: extracted.pipeline_units ?? market.pipeline_units ?? 0,
    stock_pct: market.pipeline_pct_of_stock,
  }];
}
if (extracted.submarket_median_income != null) {
  market.avg_household_income = extracted.submarket_median_income;
}
